...
class Conformer_PL(PL_Backbone):
    ...
    def set_model_parameters(self, num_labels:int):
        input_dim: int = self.settings.input.spectrogram.n_fft # self.settings.input.mel.n_mels
    ...
    def get_embeddings(self, x:torch.Tensor):
        ...
        #"""
        print("before log-Mels selection")
        x = x[:, :1]  # log-Mels only
        ...
        lengths = torch.tensor([sample.size(-2) for sample in x])   # build the tensor with the lenght of all sample in the batch
        print("lenght size: ", lengths.size())
        y, lenghts = self.model.forward(input=x, lengths=lengths)
        print("embedding size: ", y.size())
        return y