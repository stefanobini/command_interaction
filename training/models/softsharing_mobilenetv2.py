...
from torchvision.models.mobilenetv2 import ConvBNReLU, ConvBNActivation, InvertedResidual
...
class SoftSharing_PL(pl.LightningModule):
    ...
    def __init__(self, settings:DotMap, task_n_labels:List[int], task_loss_weights:np.float=None, num_classes: int = 1000, width_mult: float = 1.0, inverted_residual_setting: Optional[List[List[int]]] = None, round_nearest: int = 8, block: Optional[Callable[..., nn.Module]] = None, norm_layer: Optional[Callable[..., nn.Module]] = None):
        """
        ...
        share_inverted_residual_setting = [
            # t, c, n, s
            [1, 16, 1, 1],
            [6, 24, 2, 2],
            [6, 32, 3, 2],
        ...
        branch_inverted_residual_setting = [
            # t, c, n, s
            [6, 96, 3, 1],
            [6, 160, 3, 2],
            [6, 320, 1, 1],
        ...
        if len(share_inverted_residual_setting) == 0 or len(share_inverted_residual_setting[0]) != 4:
            raise ValueError("share_inverted_residual_setting should be non-empty "
                             "or a 4-element list, got {}".format(share_inverted_residual_setting))

        ####### SHARED BRANCH #######
        # building first layer
        input_channel = _make_divisible(input_channel * width_mult, round_nearest)
        self.last_channel = _make_divisible(last_channel * max(1.0, width_mult), round_nearest)
        shared_features: List[nn.Module] = [ConvNormActivation(3, input_channel, stride=2, norm_layer=norm_layer,
                                                        activation_layer=nn.ReLU6)]
        # building share inverted residual blocks
        for t, c, n, s in share_inverted_residual_setting:
            output_channel = _make_divisible(c * width_mult, round_nearest)
            for i in range(n):
                stride = s if i == 0 else 1
                shared_features.append(block(input_channel, output_channel, stride, expand_ratio=t, norm_layer=norm_layer))
                input_channel = output_channel
        # make it nn.Sequential
        self.shared_features = nn.Sequential(*shared_features)

        ####### BRANCH 1 #######
        branched1_features: List[nn.Module] = []
        # building share inverted residual blocks
        for t, c, n, s in branch_inverted_residual_setting:
            output_channel = _make_divisible(c * width_mult, round_nearest)
            for i in range(n):
                stride = s if i == 0 else 1
                branched1_features.append(block(input_channel, output_channel, stride, expand_ratio=t, norm_layer=norm_layer))
                input_channel = output_channel
        # building last several layers
        branched1_features.append(ConvNormActivation(input_channel, self.last_channel, kernel_size=1, norm_layer=norm_layer,
                                           activation_layer=nn.ReLU6))
        # make it nn.Sequential
        self.branched1_features = nn.Sequential(*branched1_features)
        # building classifier
        self.branch1_classifier = nn.Sequential(
            nn.Dropout(0.2),
            nn.Linear(self.last_channel, task_n_labels[0]),
        )

        ####### BRANCH 2 #######
        branched2_features: List[nn.Module] = []
        # building share inverted residual blocks
        for t, c, n, s in branch_inverted_residual_setting:
            output_channel = _make_divisible(c * width_mult, round_nearest)
            for i in range(n):
                stride = s if i == 0 else 1
                branched2_features.append(block(input_channel, output_channel, stride, expand_ratio=t, norm_layer=norm_layer))
                input_channel = output_channel
        # building last several layers
        branched2_features.append(ConvNormActivation(input_channel, self.last_channel, kernel_size=1, norm_layer=norm_layer,
                                           activation_layer=nn.ReLU6))
        # make it nn.Sequential
        self.branched2_features = nn.Sequential(*branched2_features)
        # building classifier
        self.branch2_classifier = nn.Sequential(
            nn.Dropout(0.2),
            nn.Linear(self.last_channel, task_n_labels[1]),
        )
    ...
    def forward(self, x:torch.Tensor) -> torch.Tensor:
        ...
        shared_features_map = self.shared_features(x)
        branch1_features_map = self.branched1_features(shared_features_map)
        branch2_features_map = self.branched2_features(shared_features_map)

        # Cannot use "squeeze" as batch-size can be 1
        branch1_features_map = nn.functional.adaptive_avg_pool2d(branch1_features_map, (1, 1))
        branch2_features_map = nn.functional.adaptive_avg_pool2d(branch2_features_map, (1, 1))

        branch1_embeddings = torch.flatten(branch1_features_map, 1)
        branch2_embeddings = torch.flatten(branch2_features_map, 1)

        return self.branch1_classifier(branch1_embeddings), self.branch2_classifier(branch2_embeddings)
