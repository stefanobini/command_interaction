...
settings.experimentation:str = "MTL_scr_snre"
settings.task:str = "SCR_SNRE"
if settings.task == "SI":
...
elif settings.task == "SCR_SNRE":
    settings.tasks:List[str] = ["command", "snr"]
else:
...
settings.dataset.annotations = os.path.join("datasets", "MTL_experimentation_1", "annotations")
...
settings.model.resnet8.speaker_embedding_size:int = settings.model.resnet8.out_channel      # before was multiplied by 2
...
# MobileNet V2
settings.model.mobilenetv2.speaker_embedding_size:int = settings.model.resnet8.out_channel #100
...
settings.training.checkpoint.metric_to_track:str = "val_loss"               # ["val_loss", "val_gradNorm_loss"]
settings.training.checkpoint.save_top_k:int = 1
settings.training.check_val_every_n_epoch:int = 1
settings.training.early_stop.patience:int = 10                              # default=3
...
settings.training.loss.grad_norm.alpha:float = 1.#0.5                         # Default = 0.12. For task with different level of complexity an higher value of alpha should be used to enforce the stronger training rate balancing
settings.training.loss.weights = [1.2, 0.8]

'''Noise & Curriculum Learning'''
settings.noise.min_snr:int = 0                                              # [-10, 20]
