...
HEADING = ["path", "type", "subtype", "speaker", "intent", "exp_intent", "imp_intent"]
...
def add_command_samples(data, lang_path, lang, speaker, samples):
    ...
    for sample in samples:
        ...
        intent = int(labels[0])
        exp_intent = INTENTS[intent]["explicit"][lang][int(labels[1])]["id"]
        imp_intent = INTENTS[intent]["implicit"][lang]["id"]
        data["path"].append(sample_path)
        data["type"].append(type)
        data["subtype"].append(subtype)
        data["speaker"].append(speaker)
        data["intent"].append(intent)
        data["exp_intent"].append(exp_intent)
        data["imp_intent"].append(imp_intent)
    return data
...
def add_reject_samples(path, lang, data):
    ...
    for idx in data_iter:
        ...
        data["exp_intent"].append(None)
        data["imp_intent"].append(None)
...
data = dict()
for lang in LANGS:
    data[lang] = {"path":[], "type":[], "subtype":[], "speaker":[], "intent":[], "exp_intent":[], "imp_intent":[]}

''' COMMAND SAMPLES '''
cmd_full_path = os.path.join(IN_DATASET_PATH, cmd_path)
...
for speaker in data_iter:
    ...
    for lang in LANGS:
        ...
        try:
            ...
            data[lang] = add_command_samples(data[lang], lang_path, lang, speaker, samples)
