...
PATH_DICT = {"telegram":SRC_DATASET, "google_speech":"google_speech_commands_v1", "mozilla":"mozilla_common_voices"}
...
def get_item(speech_annotations, noise_annotations, index, preprocess):
    ...
    speech_path = os.path.join("datasets", PATH_DICT[speech_item.subtype], rel_speech_path)
    ...
    return speech, settings.input.sample_rate, speech_item.path, speech_item.type, speech_item.subtype, speech_item.speaker, int(speech_item.intent), int(speech_item.explicit), int(speech_item.implicit), noise, noise_item.path, noise_item.type, noise_item.subtype
...
def build_train_set():
    ...
    for lang in LANGs:
        ...
        speech_data = {"path": list(), "type": list(), "subtype": list(), "speaker": list(), "intent": list(), "explicit": list(), "implicit": list()}
        noise_data = {"path": list(), "type": list(), "subtype": list()}
        ann_iter = tqdm(range(len(speech_annotations)))
        for idx in ann_iter:
            speech, sample_rate, speech_path, speech_type, speech_subtype, speaker, intent, explicit, implicit, noise, noise_path, noise_type, noise_subtype = get_item(speech_annotations=speech_annotations, noise_annotations=noise_annotations, index=idx, preprocess=preprocess)
            if "google" in speech_subtype:
                if speech_type == "command":
                    speech_path = os.path.join("commands", "google", speech_path.replace('/', '_'))
                else:
                    speech_path = os.path.join("rejects", lang, speech_path.replace('/', '_'))
            elif "mozilla" in speech_subtype:
                speech_path = os.path.join("rejects", speech_path)
            ...
            speech_data["intent"].append(intent)
            speech_data["explicit"].append(explicit)
            speech_data["implicit"].append(implicit)
...
def build_set(subset:str):
    ...
    for lang in LANGs:
        ...
        data = {"path": list(), "type": list(), "subtype": list(), "speaker": list(), "intent": list(), "explicit": list(), "implicit": list(), "noise_path": list(), "noise_type": list(), "noise_subtype": list(), "snr": list()}
        ann_iter = tqdm(range(len(speech_annotations)))
        for idx in ann_iter:
            speech, sample_rate, speech_path, speech_type, speech_subtype, speaker, intent, explicit, implicit, noise, noise_path, noise_type, noise_subtype = get_item(speech_annotations=speech_annotations, noise_annotations=noise_annotations, index=idx, preprocess=preprocess)
            if "google" in speech_subtype:
