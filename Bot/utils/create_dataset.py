...
from commands_dict import DEMO7_CMDS_DICT, DEMO7_CMDS_DICT_EXT, DEMO7_CMDS_DICT_EXT_2, DEMO3_CMDS_DICT, FULL_DEMO


DATASET_PATH = './saves'
#DEMO7_OUTPUT_PATH = '../training/datasets/FELICE/demo7'
#DEMO3_OUTPUT_PATH = '../training/datasets/FELICE/demo3'
FELICE_DEMO_PATH = '../training/datasets/FELICE'
            ...
            if os.path.isdir(bot_lang_path):
                ...
                for file in os.listdir(bot_lang_path):
                    if '.ogg' in file:
                        ...
                        if cmd in FULL_DEMO:
                            ds_cmd_sample_path = os.path.join(ds_cmd_lang_path, file.replace(str(cmd), str(FULL_DEMO[cmd])).replace('.ogg', '.wav'))  # update name for new command
                            ...
                            if cmd == 18:
                                ...
                                ds_cmd_sample_path = os.path.join(ds_other_lang_path, file.replace(str(cmd), str(FULL_DEMO[cmd]))).replace('.ogg', '.wav')
                                file_handle = file_ogg.export(ds_cmd_sample_path, format='wav')
