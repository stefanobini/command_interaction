...
def run_demo(req):
    ...
    if rospy.get_param("/save_speech") == True:
        with open(SPEECH_INFO_FILE, "w") as f:
            ...
            f.write(res_str)
    speech_counter += 1
    return SpeechManagerResponse(True)    # res.flag
...
if __name__ == "__main__":
    ...
    rospy.init_node('speechManager_node')
    rospy.wait_for_service('scr_service')
    # rospy.wait_for_service('speech_service')
    rospy.Service('speechManager_service', SpeechManager, run_demo)
    ...
    classify = rospy.ServiceProxy('scr_service', SCR)
